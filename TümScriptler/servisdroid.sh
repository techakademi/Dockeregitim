...
echo ""
echo -e "\e[31m--------------------------\e[0m"                                                      
echo -e "\e[1m|(:..:)ServiceDroid(:..:)|\e[0m"                                                      
echo -e "\e[31m--------------------------\e[0m" 
sleep 1
echo ""
echo -e "\e[1mServis tarama işlemine başladım, lütfen biraz bekle birazdan sonuçları görüntüleyeceğim.\e[0m"  
sleep 2
...
echo -------------------------------------------------------
echo -------------------Konteyner Bozucu--------------------
echo -------------------------------------------------------
echo ""
read -p "$Node_AD suncusunda calisan $Servis ismindeki servisin konteynerini bozmak istermisin?(E/H)" prompt
if [[ $prompt == "e" || $prompt == "E" ]]

then
    ssh root@$Node_IP "bash /bin/kontariza.sh"
    echo ""
    echo -----------------------------------------------------------
    echo $HOSTNAME Sunucusunda Servislerlin son halini listeliyorum.
    echo -----------------------------------------------------------
    echo ""
    docker service ps $Servis
else
  echo Güle Güle.
fi
